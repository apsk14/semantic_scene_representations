...
p.add_argument('--val_stat_root', required=False, help='Path to directory with statistics data.')
...
def train():
    ...
    if not opt.no_validation:
        ...
        val_dataset = dataio.SceneClassDataset(root_dir=opt.val_root,
                                                 stat_dir=((opt.stat_root).split('.')[0] + '.test'),
                                                 obj_name=opt.obj_name,
    ...
    #4489
    #5660
    model_srn = SRNsModel(num_instances=5660,
    ...
    if not opt.no_validation:
        model_srn_val = SRNsModel(num_instances=1216,
                      latent_dim=opt.embedding_size,
                      tracing_steps=opt.tracing_steps)
        model_srn_val.eval()
        model_srn_val.cuda()
    ...
    with open(os.path.join(opt.logging_root, "model.txt"), "w") as out_file:
        out_file.write(str(model_unet))

    optimizer = torch.optim.Adam(model_unet.parameters(), lr=opt.lr)
    ...
    for img_sidelength, max_steps, batch_size in zip(img_sidelengths, max_steps_per_sidelength,
        ...
        # val_dataset.set_img_sidelength(img_sidelength)
        ...
        while True:
            for model_input, ground_truth in train_dataloader:
                ...
                seg_out = model_unet(model_srn.get_output_img(model_outputs).cuda())

                optimizer.zero_grad()

                reg_loss = model_srn.get_regularization_loss(model_outputs, ground_truth)
                latent_loss = model_srn.get_latent_loss()
                class_loss = model_unet.get_seg_loss(seg_out, ground_truth)
                ...
                model_unet.write_updates(writer, model_input, seg_out, iter)
                ...
                if iter % opt.steps_til_val == 0 and not opt.no_validation:
                    ...
                    with torch.no_grad():
                        ...
                        for model_input, ground_truth in val_dataloader:
                            ...
                            seg_out = model_unet(model_srn.get_output_img(model_outputs).cuda())
                            class_loss = model_unet.get_seg_loss(seg_out, ground_truth).cpu().numpy()
                            class_losses.append(class_loss)
                            model_unet.write_updates(writer, model_input, seg_out, iter, prefix='val_')

                        writer.add_scalar("val_class_loss", np.mean(class_losses), iter)
                    model_unet.train()

                if iter % opt.steps_til_ckpt == 0:
                    util.custom_save(model_unet,
