...
from tatarchenko_autoencoder import *
import util
...
p.add_argument('--tatarchenko_ckpt', default=None, help='Path to trained model.')
p.add_argument('--linear_ckpt', default=None, help='Path to trained model.')
...
def test():
    ...
    test_set = dataio.SceneClassDatasetWithContext(root_dir=opt.data_root,
    ...
    print('Loading Tatarchenko....')
    model = TatarchenkoAutoencoder(num_classes=opt.num_classes)

    print('Loading Linear....')
    model_linear = LinearModel(num_classes=opt.num_classes)

    print("Loading model from %s" % opt.tatarchenko_ckpt)
    util.custom_load(model, path=opt.tatarchenko_ckpt, discriminator=None, overwrite_embeddings=False)
    model.eval()
    model.cuda()

    print("Loading model from %s" % opt.linear_ckpt)
    util.custom_load(model_linear, path=opt.linear_ckpt, discriminator=None, overwrite_embeddings=False)
    model_linear.eval()
    model_linear.cuda()
    ...
    with torch.no_grad():
        ...
        for model_input, ground_truth in dataset:
            ...
            model_input = {key:value.squeeze(1) if 'rgb' in key or 'seg'  in key else value for key, value in model_input.items()}
            ground_truth = {key:value.squeeze(1)  if 'rgb' in key or 'seg' in key else value for key, value in ground_truth.items()}
            ...
            comparisons = model.get_comparisons(model_input, model_outputs, ground_truth)
            ...
            for i in range(len(output_imgs)):
                ...
                newIOU = model.get_IOU_vals(pred_segs[i:i+1], trgt_segs[i:i+1], confusion, part_intersect, part_union)
