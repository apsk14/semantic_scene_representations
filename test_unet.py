...
p.add_argument('--srn_path', default=None, help='Path to trained model.')
...
def test():
    ...
    model_srn = SRNsModel(num_instances=test_set.num_instances,
                      latent_dim=opt.embedding_size,
                      tracing_steps=opt.tracing_steps)

    model_unet = UnetModel()

    assert (opt.srn_path is not None), "Have to pass checkpoint!"

    print("Loading SRN model from %s" % opt.srn_path)
    util.custom_load(model_srn, path=opt.srn_path, discriminator=None,
                     overwrite_embeddings=False)

    print("Loading Unet model from %s" % opt.unet_path)
    ...
    with torch.no_grad():
        ...
        for model_input, ground_truth in dataset:
            ...
            seg_pred = prediction.permute(0, 2, 1)
            ...
            output_imgs = model_srn.get_output_img(model_output).cpu().numpy()
            output_segs = model_srn.get_output_seg_unet(seg_pred)

            trgt_imgs = model_input['rgb'].cuda()
            trgt_segs = model_input['seg'].cuda()

            trgt_imgs = util.lin2img(trgt_imgs)
            trgt_segs_display = model_srn.get_output_seg(trgt_segs, trgt=True)
            comparisons = model_srn.get_comparisons_unet(model_input,output_imgs,output_segs,
                                                trgt_imgs,trgt_segs_display, model_output,
                                                ground_truth)
            for i in range(len(output_imgs)):
                ...
                newIOU = model_srn.get_IOU_vals_unet(seg_pred, trgt_segs, confusion, part_intersect, part_union)
                print(idx)
