...
def train():
    ...
    train_dataset = dataio.SceneClassDatasetWithContext(root_dir=opt.data_root,
    ...
    model_srn = TatarchenkoAutoencoder(opt.num_classes)
    model_srn.eval()
    model_srn.cuda()

    if opt.model_type == 'linear':
        print('Using Linear Regressor')
        model_linear = LinearModel(opt.num_classes)
    ...
    for img_sidelength, max_steps, batch_size in zip(img_sidelengths, max_steps_per_sidelength,
        ...
        while True:
            for model_input, ground_truth in train_dataloader:
                ...
                seg_out.update(model_outputs)
                model_srn.write_updates(writer, model_input, ground_truth, seg_out, iter)
