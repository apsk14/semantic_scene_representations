...
def train():
    ...
    if not opt.no_validation:
        ...
        val_dataset = dataio.SceneClassDatasetWithContext(root_dir=opt.val_root,
                                                          stat_dir=opt.stat_root,
                                                          obj_name=opt.obj_name,
    ...
    dense_optimizer = torch.optim.Adam(model.parameters(), lr=opt.lr)
    ...
    for img_sidelength, max_steps, batch_size in zip(img_sidelengths, max_steps_per_sidelength,
        ...
        while True:
            for model_input, ground_truth in train_dataloader:
                ...
                weighted_class_loss = opt.class_weight * 0.0
