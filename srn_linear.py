...
class SRNsModel(nn.Module):
    def __init__(self,
        ...
        if self.fit_single_srn:  # Fit a single scene with a single SRN (no hypernetworks)
            ...
            self.latent_codes = nn.Embedding(4489, latent_dim).cuda() #had num_instances
    ...
    def get_pc_loss(self, prediction, input):
        '''Computes loss on predicted image (L_{img} in eq. 6 in paper)
        ...
        :return: image reconstruction loss.
        '''
        _, _, _, point_cloud, _ = prediction
    ...
    def get_img_IOU_loss(self, prediction, ground_truth):
        ...
        pred_segs = pred_segs.permute(0, 2, 1)
        _, trgt_segs, _ = ground_truth
        trgt_segs = trgt_segs.permute(0, 2, 1).squeeze().cuda().long()
    ...
    def get_comparisons(self, model_input, pred, ground_truth=None):
        predictions, seg_pred, depth_maps, point_cloud,_ = pred
        colors = self.colors
        ...
        seg_pred = self.get_output_seg(pred)
        seg_pred = torch.from_numpy(seg_pred)
        # seg_pred, seg_idx = torch.max(seg_pred, dim=2)
        # seg_idx = seg_idx[:, :, None]
        # seg_pred = util.lin2img(seg_idx)
        # seg_pred = seg_pred.repeat(1,3,1,1)
        ...
        if ground_truth is not None:
            ...
            return torch.cat((normals.cpu(), predictions.cpu(), trgt_imgs.cpu(), seg_pred.cpu().float(), trgt_segs.cpu().float()), dim=3).numpy()
        else:
    ...
    def get_IOU_vals(self, prediction , trgt_seg, confusion, part_intersect, part_union): # had arg confusion
        ...
        # pred_labels = np.delete(pred_labels, np.where(trgt_seg == 0), axis=0)
        # real_label = np.delete(real_label, np.where(trgt_seg == 0), axis=0)
    ...
    def write_updates(self, writer, input, predictions, ground_truth, iter, prefix=''):
        ...
        '''
        predictions, seg_preds, depth_maps, label_pred, pts_pred = predictions
        print(pts_pred.shape)
        trgt_imgs, trgt_segs, trgt_depths = ground_truth

        observation = Observation(*input)
        pts_real = observation.pts.cuda()
        label_real = observation.labels
        ...
        seg_preds, seg_idx = torch.max(seg_preds, dim=2)
        seg_idx = seg_idx[:,:,None]
