...
class LookupLinear(nn.Module):
    def __init__(self,
        ...
        self.slave_params = nn.Embedding(num_objects, in_ch * out_ch + out_ch)

        for i in range(num_objects):
            nn.init.kaiming_normal_(self.slave_params.weight.data[i, :self.in_ch * self.out_ch].view(self.out_ch, self.in_ch),
                                    a=0.0,
                                    nonlinearity='relu',
                                    mode='fan_in')
            self.slave_params.weight.data[i, self.in_ch * self.out_ch:].fill_(0.)

    def forward(self, obj_idx):
        slave_params = self.slave_params(obj_idx)

        # Indices explicit to catch erros in shape of output layer
        weights = slave_params[..., :self.in_ch * self.out_ch]
        biases = slave_params[..., self.in_ch * self.out_ch:(self.in_ch * self.out_ch)+self.out_ch]
...
class HyperLinear(nn.Module):
    ...
    def __init__(self,
        ...
        self.slave_params = pytorch_prototyping.FCBlock(in_features=hyper_in_ch,
                                                        hidden_ch=hyper_hidden_ch,
                                                        num_hidden_layers=hyper_num_hidden_layers,
                                                        out_features=(in_ch * out_ch) + out_ch,
                                                        outermost_linear=True)
        self.slave_params[-1].apply(last_hyper_layer_init)

    def forward(self, hyper_input):
        slave_params = self.slave_params(hyper_input.cuda())
